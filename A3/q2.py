...
class SVM(object):
    ...
    def grad(self, X, y):
        ...
        zero_bias_w[0] = 0
        lam = 0.01
        return (self.c/len(X)) * np.sum(np.array([lam * zero_bias_w + loss_grad for loss_grad in hinge_loss_grads]), axis=0)
    ...
    def classify(self, X):
        ...
        classification = np.array([1 if np.dot(np.transpose(self.w), x) >= 0 else -1 for x in X])
        pre_classification = np.array([np.dot(np.transpose(self.w), x) for x in X])
        return classification, pre_classification
...
if __name__ == '__main__':
    ...
    def do_everything(beta):
        ...
        train_predictions, train_pre_class = svm.classify(train_data)
        test_predictions, test_pre_class = svm.classify(test_data)
        ...
        train_hinge_loss = hinge_loss(train_targets, train_pre_class)
        test_hinge_loss = hinge_loss(test_targets, test_pre_class)

        print("Beta == " + str(beta))
        print("Training set classification accuracy: " + str(train_accuracy))
        print("Test set classification accuracy: " + str(test_accuracy))
        print("Train hinge loss: " + str(train_hinge_loss))
        print("Test hinge loss: " + str(test_hinge_loss))
